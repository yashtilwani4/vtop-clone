...
export const AuthProvider = ({ children }) => {
    ...
    const checkAuth = async () => {
      ...
      if (token) {
        try {
          ...
          const response = await api.get('/simple-auth/me');
          dispatch({
            type: 'AUTH_SUCCESS',
            payload: {
  ...
  const login = async (credentials) => {
    try {
      ...
      // Convert userId to identifier for backend compatibility
      const loginData = {
        identifier: credentials.userId,
  ...
  const changePassword = async (passwordData) => {
    try {
      await api.post('/simple-auth/change-password', passwordData);
      return { success: true };
    } catch (error) {
